...
function salvarPrecoManual(btn) {{var td=btn.closest("td");var inp=td?td.querySelector("input[type=number]"):null;if(!inp||!inp.value){{alert("Digite um preco");return;}}var preco=parseFloat(inp.value);if(isNaN(preco)||preco<=0){{alert("Preco invalido");return;}}var parts=inp.id.split("|");var data=parts[0],sm=parts[1],nome=parts[2],emb=parts[3];var csv=data+",00:00:00,"+sm+",,,,"+nome+","+emb+",Sao Paulo,SP,Sudeste,"+preco+",,,1,,input_manual,99,1";if(navigator.clipboard){{navigator.clipboard.writeText(csv).then(function(){{inp.disabled=true;btn.disabled=true;inp.style.borderColor="green";alert("Copiado!\n"+csv);}}).catch(function(){{prompt("Copie:",csv);}});}}else{{prompt("Copie:",csv);}}}}
